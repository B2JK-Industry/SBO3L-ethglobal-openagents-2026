...
| `cargo test --workspace --tests --no-fail-fast` | ✅ **873 / 873 pass** (0 fail) across 37 test binaries on 10 crates |
...
- **Live Uniswap quote** — `UniswapExecutor::live_from_env()` (in `crates/sbo3l-execution/src/uniswap.rs`) issues a real `quoteExactInputSingle` call against the Sepolia QuoterV2 contract `0xEd1f6473345F45b75F8179591dd5bA1888cf2FB3`. Activated by `SBO3L_UNISWAP_RPC_URL` (must be a Sepolia endpoint) + `SBO3L_UNISWAP_TOKEN_OUT`; missing env vars surface as `LiveConfigError::MissingEnvVar` at construction time. The bare back-compat `UniswapExecutor::live()` ctor returns `BackendOffline` at runtime. Smoke example: `cargo run -p sbo3l-execution --example uniswap_live_smoke`.
