...
- **Standalone `mandate-keeperhub-adapter` workspace crate** (`crates/mandate-keeperhub-adapter/`, IP-4) — exposes `KeeperHubExecutor`, `KeeperHubMode`, `build_envelope`, and re-exports `GuardedExecutor` / `ExecutionReceipt` / `ExecutionError` / `MandateEnvelope` from `mandate-core`; `mandate-execution` re-exports it for back-compat. Crates.io publication remains a target, not a shipped claim.
