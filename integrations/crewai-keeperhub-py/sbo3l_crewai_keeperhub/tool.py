...
`sbo3l_crewai_keeperhub_tool(client=...)` returns a `SBO3LKeeperHubToolDescriptor`
that any tool framework taking `(name, description, callable)` accepts.
For a typed `crewai.tools.BaseTool` subclass that drops directly into a
CrewAI `Agent`'s tool list, see `crewai_tool.Sbo3lKeeperHubCrewAITool`.
"""
...
class SBO3LKeeperHubToolDescriptor:
    """Plain descriptor — wire into CrewAI via the Sbo3lKeeperHubCrewAITool
    subclass (preferred), or pass into any framework taking
...
def sbo3l_crewai_keeperhub_tool(
    *,
                ...
                return json.dumps(
                    {
                        ...
                        "detail": (
                            ...
                            "(SBO3LClientSync) to sbo3l_crewai_keeperhub_tool."
                        ),
                    }
                )
