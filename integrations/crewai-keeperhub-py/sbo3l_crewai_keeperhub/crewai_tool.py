"""Typed `crewai.tools.BaseTool` subclass for the SBO3L → KeeperHub path.

Drops directly into a CrewAI `Agent`'s tool list. Requires `crewai` as a
runtime dep — install via `pip install sbo3l-crewai-keeperhub[crewai]`.

For framework-agnostic use, prefer the plain descriptor returned by
`sbo3l_crewai_keeperhub_tool()` from `tool.py`.
"""
...
class _Sbo3lKeeperHubInput(BaseModel):
    """Schema CrewAI shows the LLM for tool args."""
...
class Sbo3lKeeperHubCrewAITool(BaseTool):  # type: ignore[misc, unused-ignore]
    """CrewAI BaseTool that gates KeeperHub execution through SBO3L.
    ...
    Example:
        ...
        # pass `tool` into your CrewAI Agent's tool list
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        descriptor = sbo3l_crewai_keeperhub_tool(
            client=client,
    ...
    async def _arun(self, aprp_json: str, **_: Any) -> str:
        ...
        # in the same crew. asyncio.to_thread offloads the call to the
        # default thread pool — codex flagged this as a P2 in the
        # langchain version; explicit override here so future maintainers
        # don't accidentally re-introduce the blocking version.
