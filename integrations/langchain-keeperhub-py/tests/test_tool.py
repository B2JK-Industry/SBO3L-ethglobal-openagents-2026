"""End-to-end test of sbo3l_keeperhub_tool against a pytest-httpx-mocked daemon.
...
from sbo3l_langchain_keeperhub import (
...
def test_allow_envelope_surfaces_kh_execution_ref(httpx_mock: HTTPXMock) -> None:
    ...
    with SBO3LClientSync("http://localhost:8730") as c:
        descriptor = sbo3l_keeperhub_tool(client=c)
...
def test_workflow_id_override(httpx_mock: HTTPXMock) -> None:
    ...
    with SBO3LClientSync("http://localhost:8730") as c:
        descriptor = sbo3l_keeperhub_tool(client=c, workflow_id=custom_workflow)
        out = json.loads(descriptor.func(APRP))
...
def test_basetool_subclass_requires_langchain() -> None:
    """If langchain-core is installed, Sbo3lKeeperHubTool is importable."""
    try:
        from sbo3l_langchain_keeperhub import Sbo3lKeeperHubTool

        assert Sbo3lKeeperHubTool is not None
    except ImportError:
        # langchain-core not installed in this env — degraded but not a failure.
        # The package's __init__ already gates the import; this test
        # confirms the gate works (no ImportError leaks into the user's
        # `from sbo3l_langchain_keeperhub import ...` for non-BaseTool
        # imports).
        pass
