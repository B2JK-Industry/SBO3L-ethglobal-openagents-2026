...
`sbo3l_keeperhub_tool(client=...)` returns a `SBO3LKeeperHubToolDescriptor`
that any tool framework taking `(name, description, callable)` accepts.
For a typed `langchain.tools.BaseTool` subclass that drops directly into
`AgentExecutor`, see `langchain_tool.Sbo3lKeeperHubTool`.
"""
...
class SBO3LKeeperHubToolDescriptor:
    ...
    or pass into any framework taking `(name, description, callable)`."""
                ...
                return json.dumps(
                    {
                        ...
                        "detail": (
                            ...
                            "(SBO3LClientSync) to sbo3l_keeperhub_tool."
                        ),
                    }
                )
