"""CrewAI tool factory for SBO3L.

The descriptor returned by `sbo3l_tool(...)` plugs into:

  - `crewai_tools.BaseTool` subclass via `from sbo3l_crewai import sbo3l_tool`
  - `crewai.tools.tool` decorator pattern (wrap the descriptor's func)
  - or any framework that accepts (name, description, callable).

We do NOT import from `crewai` here — that would make it a hard dep.
Consumers wire the descriptor into their preferred CrewAI tool primitive.
"""
...
class SBO3LToolDescriptor:
    """Plain descriptor that consumers wire into a CrewAI tool factory."""
...
def sbo3l_tool(
    *,
    ...
    """Build the SBO3L CrewAI tool descriptor.

    Wire it into CrewAI via:
        ...
        class SBO3LTool(BaseTool):
            name: str = descriptor.name
            description: str = descriptor.description
            def _run(self, aprp_json: str) -> str:
                return descriptor.func(aprp_json)

    On `deny`, the LLM sees `deny_code` (e.g. `policy.budget_exceeded`).
    Transport / auth failures surface as a JSON envelope with `error`.
    """
                ...
                return json.dumps(
                    {
                        ...
                        "detail": (
                            ...
                            "(SBO3LClientSync) to the CrewAI tool."
                        ),
                    }
                )
