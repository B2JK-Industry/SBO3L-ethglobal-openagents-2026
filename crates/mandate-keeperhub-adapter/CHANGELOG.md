...
All notable changes to `mandate-keeperhub-adapter` are recorded here. The
...
- Initial workspace release. The IP-4 realisation from
  ...
  a third-party agent framework can depend on `mandate-keeperhub-adapter`
  and get a `GuardedExecutor` plus the IP-1 `mandate_*` envelope without
  pulling the rest of the Mandate workspace.
...
- `build_envelope(&PolicyReceipt) -> MandateEnvelope` — builds the IP-1
...
- Re-exports of `GuardedExecutor`, `ExecutionError`, `ExecutionReceipt`,
  `MandateEnvelope` from `mandate_core::execution::*`.
- Quickstart example: `examples/submit_signed_receipt.rs` — runs end
  to end without touching any non-`mandate-core` workspace crate,
...
- The crate depends on `mandate-core = "0.1.0"`. Both crates are part
