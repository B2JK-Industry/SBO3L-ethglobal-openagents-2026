...
//!
//! Run with: `cargo run --example submit_signed_receipt -p mandate-keeperhub-adapter`
//!
//! This example imports ONLY `mandate_keeperhub_adapter` and
//! `mandate_core` — it doesn't touch `mandate-server`, `mandate-policy`,
//! `mandate-storage`, `mandate-cli`, or `mandate-mcp`. That's the
//! IP-4 promise: a third-party agent framework can do the same.

use mandate_core::aprp::PaymentRequest;
use mandate_core::receipt::{
    Decision, EmbeddedSignature, PolicyReceipt, ReceiptType, SignatureAlgorithm,
};
use mandate_keeperhub_adapter::{build_envelope, GuardedExecutor, KeeperHubExecutor};

fn main() {
    // 1. Build a request body. In a real agent, this is the APRP the
    //    agent sent through Mandate's HTTP / MCP surface; here we
    ...
    //    back from `mandate_server::router` after the policy +
