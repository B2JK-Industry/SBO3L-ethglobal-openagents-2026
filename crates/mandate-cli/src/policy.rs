//! `mandate policy {validate,current,activate,diff}` — local active-policy
...
use mandate_storage::policy_store::ActivateOutcome;
...
/// `mandate policy validate <file>` — parse a candidate policy and print
...
pub fn cmd_validate(path: &Path) -> ExitCode {
    match parse_and_hash(path) {
        ...
        Err(e) => {
            eprintln!("mandate policy validate: {e}");
...
/// `mandate policy current --db <path>` — print the currently-active
...
pub fn cmd_current(db: &Path) -> ExitCode {
    let storage = match open_db(db) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("mandate policy current: {e}");
        ...
        Ok(None) => {
            ...
            println!(
                "no active policy in this db. \
                 Run `mandate policy activate <file> --db {}` to seed one.",
                db.display()
            );
            ExitCode::from(3)
        }
...
/// `mandate policy activate <file> --db <path> [--source <name>]` —
...
pub fn cmd_activate(path: &Path, db: &Path, source: Option<&str>) -> ExitCode {
    let (canonical, hash, _policy) = match parse_and_hash(path) {
        Ok(t) => t,
        Err(e) => {
            eprintln!("mandate policy activate: {e}");
    ...
    match storage.policy_activate(&canonical, &hash, source_label, Utc::now()) {
        ...
        Err(e) => {
            ...
            if msg.to_lowercase().contains("unique") || msg.to_lowercase().contains("constraint") {
                eprintln!(
                    "mandate policy activate: this policy hash ({hash}) was already \
            ...
            eprintln!("mandate policy activate: {msg}");
            ExitCode::from(1)
        }
    }
}

/// `mandate policy diff <file-a> <file-b>` — diff two candidate policy
...
pub fn cmd_diff(a: &Path, b: &Path) -> ExitCode {
    ...
    let (canon_a, hash_a) = match parsed_a {
        Ok((c, h, _)) => (c, h),
        Err(e) => {
            eprintln!("mandate policy diff: {} — {e}", a.display());
    ...
    let (canon_b, hash_b) = match parsed_b {
        Ok((c, h, _)) => (c, h),
        Err(e) => {
            eprintln!("mandate policy diff: {} — {e}", b.display());
            return ExitCode::from(2);
        }
    };
    if hash_a == hash_b {
