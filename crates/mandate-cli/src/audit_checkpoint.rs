//! `mandate audit checkpoint {create,verify}` — **mock-anchored**
...
/// Stable on-disk shape of a `mandate audit checkpoint create` artifact.
...
const SCHEMA_ID: &str = "mandate.audit_checkpoint.v1";
...
pub fn cmd_create(db: &Path, out: Option<&Path>) -> ExitCode {
    let mut storage = match open_db(db) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("mandate audit checkpoint create: {e}");
    ...
    if n == 0 {
        eprintln!(
            "mandate audit checkpoint create: audit chain is empty in db {}; \
    ...
    let chain_digest = match compute_chain_digest(&hashes) {
        Ok(d) => d,
        Err(e) => {
            eprintln!("mandate audit checkpoint create: chain digest failed: {e}");
            return ExitCode::from(1);
        }
    };
    ...
    if let Some(p) = out {
        let json = match serde_json::to_string_pretty(&doc) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("mandate audit checkpoint create: serialise failed: {e}");
                return ExitCode::from(1);
            }
        };
        if let Err(e) = std::fs::write(p, json) {
            eprintln!(
                "mandate audit checkpoint create: write {} failed: {e}",
                p.display()
            );
            return ExitCode::from(1);
        }
...
/// unconditionally:
/// - schema is `mandate.audit_checkpoint.v1`
/// - `mock_anchor` is `true`
...
pub fn cmd_verify(file: &Path, db: Option<&Path>) -> ExitCode {
    let raw = match std::fs::read_to_string(file) {
        ...
        Err(e) => {
            eprintln!(
                "mandate audit checkpoint verify: read {} failed: {e}",
                file.display()
            );
            return ExitCode::from(1);
        }
    ...
    let doc: AuditCheckpointDoc = match serde_json::from_str(&raw) {
        Ok(d) => d,
        Err(e) => {
            eprintln!("mandate audit checkpoint verify: parse failed: {e}");
            return ExitCode::from(1);
        }
    };
    ...
    if doc.schema != SCHEMA_ID {
        eprintln!(
            "mandate audit checkpoint verify: bad schema id {:?} (expected {SCHEMA_ID})",
            doc.schema
        );
    ...
    if !doc.mock_anchor {
        eprintln!(
            "mandate audit checkpoint verify: mock_anchor must be true; this CLI \
             does not produce or verify real onchain anchors"
        );
        return ExitCode::from(2);
    }
    if !is_hex_64(&doc.latest_event_hash) {
        eprintln!("mandate audit checkpoint verify: latest_event_hash must be 64 hex chars");
        return ExitCode::from(2);
    }
    if !is_hex_64(&doc.chain_digest) {
        eprintln!("mandate audit checkpoint verify: chain_digest must be 64 hex chars");
        return ExitCode::from(2);
    }
    if !is_mock_anchor_ref(&doc.mock_anchor_ref) {
        eprintln!(
            "mandate audit checkpoint verify: mock_anchor_ref must match \
    ...
    if let Some(db_path) = db {
        let storage = match open_db(db_path) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("mandate audit checkpoint verify: {e}");
                return ExitCode::from(1);
            }
        };
        ...
        let hashes = match storage.audit_event_hashes_in_order() {
            Ok(h) => h,
            Err(e) => {
                eprintln!("mandate audit checkpoint verify: read chain: {e}");
                return ExitCode::from(1);
            }
        };
        let computed = match compute_chain_digest(&hashes) {
            Ok(d) => d,
            Err(e) => {
                eprintln!("mandate audit checkpoint verify: chain digest: {e}");
                return ExitCode::from(1);
            }
        };
        ...
        let row = match storage.audit_checkpoint_by_anchor_ref(&doc.mock_anchor_ref) {
            Ok(r) => r,
            Err(e) => {
                eprintln!("mandate audit checkpoint verify: lookup: {e}");
                return ExitCode::from(1);
            }
        };
        let row = match row {
            ...
            None => {
                eprintln!(
                    "mandate audit checkpoint verify: no row in {} matches \
        ...
        if row.chain_digest != doc.chain_digest {
            eprintln!(
                "mandate audit checkpoint verify: chain_digest mismatch (db={}, doc={}); \
        ...
        if row.latest_event_hash != doc.latest_event_hash {
            eprintln!(
                "mandate audit checkpoint verify: latest_event_hash mismatch (db={}, doc={})",
        ...
        if computed != doc.chain_digest {
            ...
            let prefix_digest = match compute_chain_digest(&prefix) {
                Ok(d) => d,
                Err(e) => {
                    eprintln!("mandate audit checkpoint verify: prefix digest: {e}");
                    return ExitCode::from(1);
                }
            };
            if prefix_digest == doc.chain_digest {
                ...
                eprintln!(
                    "mandate audit checkpoint verify: chain prefix through seq={} \
