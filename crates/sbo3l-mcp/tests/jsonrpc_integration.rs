//! Integration tests for the `sbo3l-mcp` stdio JSON-RPC server (Passport P3.1).
//!
//! Two layers:
//! 1. **In-process dispatch** — calls `sbo3l_mcp::dispatch` directly. Exercises
...
//! 2. **Stdio child-process** — spawns the `sbo3l-mcp` binary, writes
...
//!   `Storage::policy_activate` rather than spawning `sbo3l policy activate`,
...
use sbo3l_mcp::{dispatch, error_codes, ServerContext, TOOL_ERROR_CODE};
...
// Public dev signers from `sbo3l_server::AppState::new`. These match the
...
fn dev_pubkeys_match_canonical_constants() {
    ...
    // seeds in `crates/sbo3l-server/src/lib.rs::AppState::new`, this
...
fn fresh_db() -> (tempfile::TempDir, PathBuf, ServerContext) {
    ...
    let path = dir.path().join("sbo3l.sqlite");
    ...
    // hermetic without racing on the process-global `SBO3L_MCP_ROOT`
    // env var.
...
fn activate_reference_policy(db_path: &Path) {
    use sbo3l_core::hashing::{canonical_json, sha256_hex};
...
fn validate_aprp_accepts_golden_fixture() {
    ...
    let result = dispatch("sbo3l.validate_aprp", &json!({ "aprp": aprp }), &ctx).unwrap();
...
fn validate_aprp_rejects_unknown_field_with_stable_code() {
    ...
    let err = dispatch("sbo3l.validate_aprp", &json!({ "aprp": aprp }), &ctx).unwrap_err();
...
fn validate_aprp_rejects_missing_field_param() {
    let ctx = ServerContext::new();
    let err = dispatch("sbo3l.validate_aprp", &json!({}), &ctx).unwrap_err();
...
fn decide_without_active_policy_returns_policy_not_active() {
    ...
    let _ = sbo3l_storage::Storage::open(&db).expect("init empty db");
    let aprp = read_json(APRP_ALLOW);
    let ctx = ServerContext::with_root(dir.path().to_path_buf());
    let err = dispatch(
        "sbo3l.decide",
...
// ---------------------------------------------------------------------------
// Tool 3 — sbo3l.run_guarded_execution
// ---------------------------------------------------------------------------
...
fn verify_capsule_accepts_golden_fixture_via_path() {
    ...
    .unwrap();
    assert_eq!(result["ok"], true);
    assert_eq!(result["schema"], "sbo3l.passport_capsule.v1");
}
...
fn verify_capsule_accepts_inline_object() {
    ...
    let result = dispatch("sbo3l.verify_capsule", &json!({ "capsule": capsule }), &ctx).unwrap();
    assert_eq!(result["ok"], true);
}

// ---------------------------------------------------------------------------
// Tool 5 — sbo3l.explain_denial
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Tool 6 — sbo3l.audit_lookup (IP-3)
// ---------------------------------------------------------------------------
...
fn audit_lookup_hit_returns_bundle_for_seeded_audit_event() {
    ...
    assert_eq!(bundle["bundle_type"], "sbo3l.audit_bundle.v1");
...
fn jsonrpc_error_envelope_carries_stable_code_in_data() {
    use sbo3l_mcp::dispatch_to_response;
    use sbo3l_mcp::jsonrpc::Request;
    use std::sync::Arc;
...
impl McpServer {
    ...
    fn spawn_with_env(extra_env: &[(&str, &Path)]) -> Self {
        ...
        let child = cmd.spawn().expect("spawn sbo3l-mcp");
        Self { child }
    }
...
fn stdio_tools_list_round_trips_through_child_process() {
    ...
    assert!(names.contains(&"sbo3l.audit_lookup"));
}
...
fn stdio_validate_aprp_round_trips_through_child_process() {
    let aprp = read_json(APRP_ALLOW);
    let mut server = McpServer::spawn();
    let resp = server.call("sbo3l.validate_aprp", json!({ "aprp": aprp }));
...
fn stdio_decide_allow_round_trips_through_child_process() {
    ...
    // Round 0 path-sandbox: child reads SBO3L_MCP_ROOT from env.
    // Pass the tempdir so the db path resolves inside the sandbox.
    let mut server = McpServer::spawn_with_env(&[("SBO3L_MCP_ROOT", dir.path())]);
...
fn mcp_rejects_path_outside_root() {
    ...
    let _ = sbo3l_storage::Storage::open(&escape_db).expect("seed db");
    ...
    assert!(
        err.message.contains("escapes SBO3L_MCP_ROOT"),
...
fn mcp_rejects_path_traversal_via_dotdot() {
    ...
    let _ = sbo3l_storage::Storage::open(&outside_target).expect("seed db");
...
fn mcp_rejects_symlink_escape() {
    ...
    let _ = sbo3l_storage::Storage::open(&target).expect("seed target");
...
fn mcp_uses_cwd_when_root_unset() {
    ...
    let prior = std::env::var("SBO3L_MCP_ROOT").ok();
    ...
    unsafe {
        std::env::remove_var("SBO3L_MCP_ROOT");
    }
    let resolved = ctx.effective_root();
    if let Some(p) = prior {
        unsafe {
            std::env::set_var("SBO3L_MCP_ROOT", p);
        }
    }
...
// crates/sbo3l-cli/tests/passport_cli.rs; the tests below pin the
...
fn run_guarded_rejects_requires_human_with_stable_code() {
    ...
    // sbo3l.passport_capsule.v1, so the tool refuses. This pins the
    ...
    // end-to-end belongs to crates/sbo3l-cli/tests/passport_cli.rs.
