//! SBO3L MCP — stdio JSON-RPC server (Passport P3.1).
//!
//! This crate exposes SBO3L's policy + capsule + audit primitives as
...
//! | `sbo3l.explain_denial` | `sbo3l_core::passport::verify_capsule` + structured projection |
//! | `sbo3l.audit_lookup` | `Storage::audit_chain_prefix_through` + `sbo3l_core::audit_bundle::build` (IP-3) |
//!
...
//! `sbo3l.run_guarded_execution` accepts only `mode: "mock"`.

use std::path::{Path, PathBuf};
use std::sync::Arc;

use sbo3l_core::audit_bundle::{self, BundleError};
use sbo3l_core::error::SchemaError;
use sbo3l_core::passport::{verify_capsule, CapsuleVerifyError};
use sbo3l_core::receipt::PolicyReceipt;
use sbo3l_core::schema::{validate_aprp, validate_passport_capsule};
...
pub mod error_codes {
    ...
    /// `SBO3L_MCP_ROOT` sandbox (or its symlink-resolved canonical
...
/// reads `SBO3L_MCP_ROOT` from the environment, falling back to the
    ...
    pub fn effective_root(&self) -> Result<PathBuf, ToolError> {
        ...
        std::env::current_dir().map_err(|e| {
            ToolError::new(
                ...
                format!("SBO3L_MCP_ROOT unset and current_dir() failed: {e}"),
            )
        })
    }
}
...
pub fn canonicalize_within_root(path: &Path, root: &Path) -> Result<PathBuf, ToolError> {
    let canonical_root = root.canonicalize().map_err(|e| {
        ToolError::new(
            error_codes::PATH_ESCAPE,
            format!(
                "SBO3L_MCP_ROOT {} could not be canonicalized: {e}",
                root.display()
            ),
        )
    })?;
    ...
    vec![
        ...
        ToolDescriptor {
            ...
            input_schema: json!({
                ...
                "properties": {
                    "aprp": { "type": "object" },
                    "db":   { "type": "string", "description": "filesystem path to a SBO3L SQLite DB" }
                },
                "additionalProperties": false
            }),
            output_schema: json!({
                "type": "object",
                "description": "sbo3l-server PaymentRequestResponse",
                "additionalProperties": true
            }),
        },
        ToolDescriptor {
            name: "sbo3l.run_guarded_execution",
            description: "Run sbo3l.decide + (allow path) the chosen mock executor (KeeperHub or \
        ...
        ToolDescriptor {
            ...
            output_schema: json!({
                ...
                "properties": {
                    "ok":     { "const": true },
                    "schema": { "const": "sbo3l.passport_capsule.v1" }
                },
                "required": ["ok", "schema"],
                "additionalProperties": false
            }),
        ...
        ToolDescriptor {
            name: "sbo3l.audit_lookup",
            description:
                "IP-3 sister tool. Given a SBO3L audit_event_id + a signed PolicyReceipt + \
                 the DB path + signer pubkeys, returns the corresponding sbo3l.audit_bundle.v1. \
            ...
            output_schema: json!({
                ...
                "properties": {
                    "ok":     { "const": true },
                    "bundle": { "type": "object", "description": "sbo3l.audit_bundle.v1" }
                },
                "required": ["ok", "bundle"],
                "additionalProperties": false
            }),
...
/// matches the in-process pattern `sbo3l passport run` already uses
/// (`crates/sbo3l-cli/src/passport.rs::cmd_run` step 6).
pub fn dispatch(method: &str, params: &Value, ctx: &ServerContext) -> Result<Value, ToolError> {
    match method {
        ...
        "sbo3l.validate_aprp" => tool_validate_aprp(params),
        "sbo3l.decide" => tool_decide(params, ctx),
        "sbo3l.run_guarded_execution" => tool_run_guarded(params, ctx),
        "sbo3l.verify_capsule" => tool_verify_capsule(params, ctx),
        "sbo3l.explain_denial" => tool_explain_denial(params, ctx),
        "sbo3l.audit_lookup" => tool_audit_lookup(params, ctx),
        _ => Err(ToolError::new(
            error_codes::PARAMS_INVALID,
...
fn tool_validate_aprp(params: &Value) -> Result<Value, ToolError> {
    ...
    let bytes = sbo3l_core::hashing::canonical_json(aprp).map_err(|e| {
        ToolError::new(
            error_codes::APRP_INVALID,
            format!("APRP is schema-valid but could not be canonicalised: {e}"),
        )
    })?;
    let request_hash = sbo3l_core::hashing::sha256_hex(&bytes);
...
fn tool_run_guarded(params: &Value, ctx: &ServerContext) -> Result<Value, ToolError> {
    ...
    if mode != "mock" {
        return Err(ToolError::new(
            ...
            "sbo3l.run_guarded_execution only supports mode=mock in P3.1; live integration \
    ...
    if matches!(
        ...
        return Err(ToolError::new(
            ...
            "policy returned requires_human; sbo3l.run_guarded_execution does not encode that \
             outcome (sbo3l.passport_capsule.v1 only allows allow/deny). Use the regular API \
    ...
    let decision_block = json!({
        "result": match response.receipt.decision {
            ...
            sbo3l_core::receipt::Decision::RequiresHuman => unreachable!("rejected above"),
        },
...
fn tool_verify_capsule(params: &Value, ctx: &ServerContext) -> Result<Value, ToolError> {
    ...
    Ok(json!({ "ok": true, "schema": "sbo3l.passport_capsule.v1" }))
}
...
fn tool_explain_denial(params: &Value, ctx: &ServerContext) -> Result<Value, ToolError> {
    ...
    if result != "deny" {
        return Err(ToolError::new(
            ...
            format!(
                "sbo3l.explain_denial only operates on deny capsules; this capsule is `{result}`"
            ),
        ));
    }
    let projection = json!({
...
fn tool_audit_lookup(params: &Value, ctx: &ServerContext) -> Result<Value, ToolError> {
    ...
    let chain = match storage.audit_chain_prefix_through(event_id) {
        Ok(c) => c,
        Err(sbo3l_storage::error::StorageError::AuditEventNotFound { id }) => {
            return Err(ToolError::new(
    ...
    let active = storage
        ...
        .ok_or_else(|| {
            ToolError::new(
                ...
                format!(
                    "no active policy in {}; run `sbo3l policy activate` first",
                    db_path.display()
                ),
            )
        })?;
...
fn capsule_error_code(_err: &CapsuleVerifyError) -> &'static str {
    ...
    // exactly the way `sbo3l passport verify` callers do.
    error_codes::CAPSULE_INVALID
}
