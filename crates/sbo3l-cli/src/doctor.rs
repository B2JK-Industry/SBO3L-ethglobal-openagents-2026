//! `sbo3l doctor` — operator readiness summary.
//!
...
use sbo3l_core::audit::verify_chain;
...
struct DoctorReport {
    /// Always `"sbo3l.doctor.v1"`. Stable identifier so consumers can
...
fn run_checks(storage: &Storage) -> Vec<Check> {
    ...
    //     hash prefix (full hash in `sbo3l policy current`).
    //   - table present + no active row: `skip` ("table is here, no
    //     policy seeded yet — run `sbo3l policy activate <file>`").
    ...
    match storage.optional_count("active_policy") {
        Ok(Some(_)) => match storage.policy_current() {
            ...
            Ok(None) => checks.push(skip(
                ...
                "table present but no policy activated yet — run \
                 `sbo3l policy activate <file> --db <path>` to seed one (PSM-A3)",
            )),
        ...
        Ok(None) => checks.push(skip(
            ...
            "table not present — older daemon DB before V006; the daemon \
             still uses the embedded reference policy until `sbo3l policy \
    ...
    //     (full ref via `sbo3l audit checkpoint create --out` or
    //     storage list).
    //   - table present + no rows: `skip` ("no checkpoint created
    //     yet — run `sbo3l audit checkpoint create`"). Truthful:
        ...
        Ok(Some(_)) => match storage.audit_checkpoint_latest() {
            ...
            Ok(None) => checks.push(skip(
                ...
                "table present but no checkpoint created yet — run \
                 `sbo3l audit checkpoint create --db <path>` to anchor the \
...
fn render_human(report: &DoctorReport) -> String {
    ...
    use std::fmt::Write;
    let _ = writeln!(out, "sbo3l doctor — operator readiness summary");
...
pub fn run(db: Option<&Path>, json: bool) -> ExitCode {
    ...
    let storage = match storage_result {
        ...
        Err(e) => {
            ...
            if json {
                ...
                eprintln!("sbo3l doctor: failed to open db {path}: {e}");
            }
            return ExitCode::from(2);
        }
    };
    ...
    if json {
        match serde_json::to_string_pretty(&report) {
            Ok(s) => println!("{s}"),
            Err(e) => {
                eprintln!("sbo3l doctor: failed to serialise report: {e}");
                return ExitCode::from(2);
            }
        }
    } else {
...
mod tests {
    ...
    fn active_policy_skip_when_no_policy_seeded() {
        ...
        // skip with a pointer at `sbo3l policy activate`, not as
        ...
        match &row.status {
            CheckStatus::Skip { reason } => {
                assert!(
                    ...
                    "skip reason must point at `sbo3l policy activate` and PSM-A3; got: {reason}"
                );
            }
    ...
    fn active_policy_skip_when_table_missing_on_older_db() {
        ...
        // operators at the V006 migration / `sbo3l policy activate`
        // flow.
    ...
    fn audit_checkpoints_skip_when_no_checkpoint_seeded() {
        ...
        // surface skip with a pointer at `sbo3l audit checkpoint
    ...
    fn audit_checkpoints_reports_ok_after_create() {
        ...
        // mock_anchor_ref tail. Full ref is available via `sbo3l
    ...
    fn json_envelope_serialises_with_stable_report_type() {
        ...
        assert_eq!(v["report_type"], "sbo3l.doctor.v1");
