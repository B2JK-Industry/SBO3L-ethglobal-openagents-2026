//! `sbo3l key {init,list,rotate} --mock` — production-shaped mock KMS
...
use chrono::Utc;
use sbo3l_core::mock_kms;
use sbo3l_storage::mock_kms_store::MockKmsKeyRecord;
...
/// `sbo3l key init --mock --role <name> --root-seed <hex64> [--genesis <ts>] --db <path>`
///
...
pub fn cmd_init(
    ...
    let genesis_ts = match genesis {
        Some(g) => match chrono::DateTime::parse_from_rfc3339(g) {
            ...
            Err(e) => {
                eprintln!("sbo3l key init: --genesis must be RFC3339: {e}");
                return ExitCode::from(2);
            }
        },
        None => Utc::now(),
            ...
            match storage.mock_kms_list(Some(role)) {
                Ok(rows) if !rows.is_empty() => {
                    ...
                    match v1 {
                        ...
                        None => {
                            eprintln!(
                                "sbo3l key init: existing rows for role={role} but no v1; \
                ...
                Ok(_) => {
                    eprintln!(
                        "sbo3l key init: insert was rejected as duplicate but no rows \
...
/// `sbo3l key list --mock [--role <name>] --db <path>`
///
...
/// `sbo3l key rotate --mock --role <name> --root-seed <hex64> --db <path>`
///
...
pub fn cmd_rotate(mock: bool, role: &str, root_seed_hex: &str, db: &Path) -> ExitCode {
    ...
    let current_version = match current {
        ...
        None => {
            eprintln!(
                "sbo3l key rotate: no keyring exists for role={role}. Run \
                 `sbo3l key init --mock --role {role} --root-seed <hex64> --db {} ` first.",
                db.display()
            );
            return ExitCode::from(1);
        }
    ...
    let stored_current = match storage.mock_kms_list(Some(role)) {
        ...
        Err(e) => {
            eprintln!("sbo3l key rotate: failed to read existing keyring: {e}");
            return ExitCode::from(1);
        }
    };
    let stored_current = match stored_current {
        ...
        None => {
            ...
            eprintln!(
                "sbo3l key rotate: db inconsistency: current_version={current_version} \
    ...
    {
        eprintln!(
            "sbo3l key rotate: --root-seed does not match the seed that produced \
             the stored v{current_version} for role={role}; refusing to rotate. \
             Pass the same --root-seed used at `sbo3l key init` (or the most \
             recent `sbo3l key rotate`)."
        );
        return ExitCode::from(2);
    }
    ...
    match storage.mock_kms_insert(&record) {
        ...
        Ok(false) => {
            eprintln!(
                "sbo3l key rotate: insert rejected — likely a parallel rotate \
...
// Integration tests live in `crates/sbo3l-cli/tests/key_cli.rs` so they
// can reach the cargo-built binary via `env!("CARGO_BIN_EXE_sbo3l")`.
