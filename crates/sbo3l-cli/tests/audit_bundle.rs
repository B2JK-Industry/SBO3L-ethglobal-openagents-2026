//! Integration smoke test for `sbo3l audit export` → `sbo3l audit verify-bundle`.
//!
...
// ----- DB-backed export ---------------------------------------------------
//
// The DB-backed export reads the audit chain directly from a SBO3L
// daemon's SQLite store via `sbo3l_storage::Storage`. The fixture below
...
fn build_db_fixture() -> (
    ...
    let tmp = tempfile::tempdir().unwrap();
    let db_path = tmp.path().join("sbo3l.sqlite");
    let audit_signer = DevSigner::from_seed("audit-signer-v1", [11u8; 32]);
    let receipt_signer = DevSigner::from_seed("decision-signer-v1", [7u8; 32]);
...
fn db_backed_export_then_verify_bundle_succeeds() {
    ...
    // round-trips clean. This is the "SBO3L leaves behind verifiable
