//! Integration tests for `sbo3l policy {validate,current,activate,diff}`
//! (PSM-A3).
//!
...
fn validate_prints_hash_and_summary_for_valid_policy() {
    ...
    // same constant baked into `crates/sbo3l-identity/src/ens.rs:89`.
    assert!(
...
fn current_with_no_active_policy_returns_exit_three() {
    ...
    // it. `sbo3l key list --mock --db <path>` opens the DB, runs
