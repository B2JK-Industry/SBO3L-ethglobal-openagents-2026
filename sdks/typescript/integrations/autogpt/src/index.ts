/**
 * `@sbo3l/autogpt` — SBO3L adapter for the AutoGPT agent framework.
 *
 * Wraps `@sbo3l/sdk`'s `SBO3LClient.submit()` in a AutoGPT-shaped
 ...
 *
 *   // Hand `tool.descriptor` to AutoGPT's tool registry; pair
 *   // each tool call with `runSbo3lCommand` to dispatch through SBO3L.
 *   ```
 */
...
export const DEFAULT_COMMAND_NAME = "sbo3l_payment_request";
...
/** APRP v1 input schema for AutoGPT's tool descriptor. Hand-authored — no schema-gen dep. */
...
export interface AutoGPTDescriptor {
  name: string;
  description: string;
  schema: typeof APRP_SCHEMA;
}

export interface AutoGPTToolCall {
  id: string;
  name: string;
  /** JSON-encoded APRP. */
  input: string;
...
export interface SBO3LAutoGPTTool {
  descriptor: AutoGPTDescriptor;
  name: string;
  /**
...
export function sbo3lCommand(options: SBO3LToolOptions): SBO3LAutoGPTTool {
  const { client } = options;
  const name = options.name ?? DEFAULT_COMMAND_NAME;
...
/**
 * Handle one AutoGPT tool call. Parses the JSON input, dispatches
 ...
 */
export async function runSbo3lCommand(
  tool: SBO3LAutoGPTTool,
  call: AutoGPTToolCall,
): Promise<AutoGPTToolResult> {
