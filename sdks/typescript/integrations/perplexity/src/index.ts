/**
 * `@sbo3l/perplexity` — SBO3L adapter for the Perplexity agent / API surface.
 *
 ...
 *
 *   // Wire `tool.descriptor` into Perplexity's tool registry; pair
 *   // each emitted tool call with `runSbo3lPerplexityToolCall`.
 *   ```
 */
...
/** APRP v1 input schema for Perplexity's tool descriptor. Hand-authored — no schema-gen dep. */
...
export interface PerplexityDescriptor {
  name: string;
  description: string;
...
export interface PerplexityToolResult {
  tool_call_id: string;
  ok: boolean;
...
export interface SBO3LPerplexityTool {
...
export function sbo3lPerplexityTool(options: SBO3LToolOptions): SBO3LPerplexityTool {
...
export async function runSbo3lPerplexityToolCall(
  tool: SBO3LPerplexityTool,
  call: PerplexityToolCall,
): Promise<PerplexityToolResult> {
