...
import {
  ...
  PolicyDenyError,
  sbo3lPerplexityTool,
  runSbo3lPerplexityToolCall,
  type PaymentRequest,
  type PerplexityToolCall,
} from "../src/index.js";
...
describe("sbo3lPerplexityTool", () => {
  it("default tool name", () => {
    const t = sbo3lPerplexityTool({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
...
describe("runSbo3lPerplexityToolCall", () => {
  function call(name: string, input: object | string, id = "tc-001"): PerplexityToolCall {
  ...
  it("forwards allow into ok=true envelope", async () => {
    fetchOnce(allow("evt-1"));
    const t = sbo3lPerplexityTool({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
    const out = await runSbo3lPerplexityToolCall(t, call(t.name, aprpFixture));
  ...
  it("rejects malformed JSON input", async () => {
    const t = sbo3lPerplexityTool({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
    const out = await runSbo3lPerplexityToolCall(t, call(t.name, "{not-json"));
    expect(out.ok).toBe(false);
    expect(JSON.parse(out.output).error).toBe("input.bad_arguments");
  });

  it("rejects mismatched tool name", async () => {
    const t = sbo3lPerplexityTool({ client: new SBO3LClient({ endpoint: ENDPOINT }), name: "pay" });
    const out = await runSbo3lPerplexityToolCall(t, call("not_pay", aprpFixture));
    expect(out.ok).toBe(false);
