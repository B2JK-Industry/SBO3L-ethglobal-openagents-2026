...
import {
  APRP_SCHEMA,
  PolicyDenyError,
  sbo3lReplicatePredictionGate,
  runSbo3lReplicatePredictionGate,
  type PaymentRequest,
  type ReplicateToolCall,
} from "../src/index.js";
...
describe("sbo3lReplicatePredictionGate", () => {
  it("default tool name", () => {
    const t = sbo3lReplicatePredictionGate({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
  ...
  it("name + description overrides flow into descriptor", () => {
    const t = sbo3lReplicatePredictionGate({
...
describe("runSbo3lReplicatePredictionGate", () => {
  function call(name: string, input: object | string, id = "tc-001"): ReplicateToolCall {
  ...
  it("forwards allow into ok=true envelope", async () => {
    fetchOnce(allow("evt-1"));
    const t = sbo3lReplicatePredictionGate({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
    const out = await runSbo3lReplicatePredictionGate(t, call(t.name, aprpFixture));
    expect(out.ok).toBe(true);
    expect(out.tool_call_id).toBe("tc-001");
    expect(JSON.parse(out.output).execution_ref).toBe("kh-1");
  });
  ...
  it("rejects malformed JSON input", async () => {
    const t = sbo3lReplicatePredictionGate({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
    const out = await runSbo3lReplicatePredictionGate(t, call(t.name, "{not-json"));
    expect(out.ok).toBe(false);
    expect(JSON.parse(out.output).error).toBe("input.bad_arguments");
  });

  it("rejects mismatched tool name", async () => {
    const t = sbo3lReplicatePredictionGate({ client: new SBO3LClient({ endpoint: ENDPOINT }), name: "pay" });
    const out = await runSbo3lReplicatePredictionGate(t, call("not_pay", aprpFixture));
    expect(out.ok).toBe(false);
