/**
 * `@sbo3l/replicate` — SBO3L adapter for the Replicate agent / API surface.
 *
 ...
 *
 *   // Wire `tool.descriptor` into Replicate's tool registry; pair
 *   // each emitted tool call with `runSbo3lReplicatePredictionGate`.
 *   ```
 */
...
/** APRP v1 input schema for Replicate's tool descriptor. Hand-authored — no schema-gen dep. */
...
export interface ReplicateDescriptor {
  name: string;
  description: string;
  schema: typeof APRP_SCHEMA;
}

export interface ReplicateToolCall {
  id: string;
  name: string;
  /** JSON-encoded APRP. */
  input: string;
...
export interface SBO3LReplicateTool {
  descriptor: ReplicateDescriptor;
  name: string;
  /** Throws `PolicyDenyError` on deny / requires_human, returns receipt on allow. */
  execute: (aprp: PaymentRequest) => Promise<PolicyReceipt>;
}

export function sbo3lReplicatePredictionGate(options: SBO3LToolOptions): SBO3LReplicateTool {
...
export async function runSbo3lReplicatePredictionGate(
  tool: SBO3LReplicateTool,
  call: ReplicateToolCall,
