...
import {
  ...
  PolicyDenyError,
  sbo3lSuperAgiTool,
  runSbo3lSuperAgiTool,
  type PaymentRequest,
  type SuperAGIToolCall,
} from "../src/index.js";
...
describe("sbo3lSuperAgiTool", () => {
  it("default tool name", () => {
    const t = sbo3lSuperAgiTool({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
...
describe("runSbo3lSuperAgiTool", () => {
  function call(name: string, input: object | string, id = "tc-001"): SuperAGIToolCall {
  ...
  it("forwards allow into ok=true envelope", async () => {
    fetchOnce(allow("evt-1"));
    const t = sbo3lSuperAgiTool({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
    const out = await runSbo3lSuperAgiTool(t, call(t.name, aprpFixture));
  ...
  it("rejects malformed JSON input", async () => {
    const t = sbo3lSuperAgiTool({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
    const out = await runSbo3lSuperAgiTool(t, call(t.name, "{not-json"));
    expect(out.ok).toBe(false);
    expect(JSON.parse(out.output).error).toBe("input.bad_arguments");
  });

  it("rejects mismatched tool name", async () => {
    const t = sbo3lSuperAgiTool({ client: new SBO3LClient({ endpoint: ENDPOINT }), name: "pay" });
    const out = await runSbo3lSuperAgiTool(t, call("not_pay", aprpFixture));
    expect(out.ok).toBe(false);
