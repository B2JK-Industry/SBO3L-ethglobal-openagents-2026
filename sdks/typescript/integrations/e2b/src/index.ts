/**
 * `@sbo3l/e2b` — SBO3L adapter for the E2B agent / API surface.
 *
 ...
 *
 *   // Wire `tool.descriptor` into E2B's tool registry; pair
 *   // each emitted tool call with `runSbo3lE2BGate`.
 *   ```
 */
...
/** APRP v1 input schema for E2B's tool descriptor. Hand-authored — no schema-gen dep. */
...
export interface E2BDescriptor {
  name: string;
  description: string;
  schema: typeof APRP_SCHEMA;
}

export interface E2BToolCall {
  id: string;
  name: string;
  /** JSON-encoded APRP. */
  input: string;
...
export interface SBO3LE2BTool {
  descriptor: E2BDescriptor;
  name: string;
  /** Throws `PolicyDenyError` on deny / requires_human, returns receipt on allow. */
  execute: (aprp: PaymentRequest) => Promise<PolicyReceipt>;
}

export function sbo3lE2BGate(options: SBO3LToolOptions): SBO3LE2BTool {
...
export async function runSbo3lE2BGate(
  tool: SBO3LE2BTool,
  call: E2BToolCall,
