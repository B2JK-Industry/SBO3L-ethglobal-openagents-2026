/**
 * `@sbo3l/agentforce` — SBO3L adapter for the Agentforce agent / API surface.
 *
 ...
 *
 *   // Wire `tool.descriptor` into Agentforce's tool registry; pair
 *   // each emitted tool call with `runSbo3lAgentforceAction`.
 *   ```
 */
...
/** APRP v1 input schema for Agentforce's tool descriptor. Hand-authored — no schema-gen dep. */
...
export interface AgentforceToolCall {
  id: string;
  name: string;
  /** JSON-encoded APRP. */
  input: string;
...
export interface AgentforceToolResult {
  tool_call_id: string;
  ok: boolean;
...
export interface SBO3LAgentforceTool {
  descriptor: AgentforceDescriptor;
  name: string;
  /** Throws `PolicyDenyError` on deny / requires_human, returns receipt on allow. */
  execute: (aprp: PaymentRequest) => Promise<PolicyReceipt>;
}

export function sbo3lAgentforceAction(options: SBO3LToolOptions): SBO3LAgentforceTool {
...
export async function runSbo3lAgentforceAction(
  tool: SBO3LAgentforceTool,
  call: AgentforceToolCall,
): Promise<AgentforceToolResult> {
