...
import {
  APRP_SCHEMA,
  PolicyDenyError,
  sbo3lCopilotStudioAction,
  runSbo3lCopilotStudioAction,
  type PaymentRequest,
  type CopilotStudioToolCall,
} from "../src/index.js";
...
describe("sbo3lCopilotStudioAction", () => {
  it("default tool name", () => {
    const t = sbo3lCopilotStudioAction({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
  ...
  it("name + description overrides flow into descriptor", () => {
    const t = sbo3lCopilotStudioAction({
...
describe("runSbo3lCopilotStudioAction", () => {
  function call(name: string, input: object | string, id = "tc-001"): CopilotStudioToolCall {
  ...
  it("forwards allow into ok=true envelope", async () => {
    fetchOnce(allow("evt-1"));
    const t = sbo3lCopilotStudioAction({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
    const out = await runSbo3lCopilotStudioAction(t, call(t.name, aprpFixture));
  ...
  it("rejects malformed JSON input", async () => {
    const t = sbo3lCopilotStudioAction({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
    const out = await runSbo3lCopilotStudioAction(t, call(t.name, "{not-json"));
    expect(out.ok).toBe(false);
    expect(JSON.parse(out.output).error).toBe("input.bad_arguments");
  });

  it("rejects mismatched tool name", async () => {
    const t = sbo3lCopilotStudioAction({ client: new SBO3LClient({ endpoint: ENDPOINT }), name: "pay" });
    const out = await runSbo3lCopilotStudioAction(t, call("not_pay", aprpFixture));
    expect(out.ok).toBe(false);
