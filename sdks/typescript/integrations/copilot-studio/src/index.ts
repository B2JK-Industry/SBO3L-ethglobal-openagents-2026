/**
 * `@sbo3l/copilot-studio` — SBO3L adapter for the CopilotStudio agent / API surface.
 *
 ...
 *
 *   // Wire `tool.descriptor` into CopilotStudio's tool registry; pair
 *   // each emitted tool call with `runSbo3lCopilotStudioAction`.
 *   ```
 */
...
/** APRP v1 input schema for CopilotStudio's tool descriptor. Hand-authored — no schema-gen dep. */
...
export interface CopilotStudioDescriptor {
  name: string;
  description: string;
  schema: typeof APRP_SCHEMA;
}

export interface CopilotStudioToolCall {
  id: string;
  name: string;
  /** JSON-encoded APRP. */
  input: string;
...
export interface CopilotStudioToolResult {
  tool_call_id: string;
  ok: boolean;
...
export interface SBO3LCopilotStudioTool {
  descriptor: CopilotStudioDescriptor;
  name: string;
  /** Throws `PolicyDenyError` on deny / requires_human, returns receipt on allow. */
  execute: (aprp: PaymentRequest) => Promise<PolicyReceipt>;
}

export function sbo3lCopilotStudioAction(options: SBO3LToolOptions): SBO3LCopilotStudioTool {
...
export async function runSbo3lCopilotStudioAction(
  tool: SBO3LCopilotStudioTool,
  call: CopilotStudioToolCall,
): Promise<CopilotStudioToolResult> {
