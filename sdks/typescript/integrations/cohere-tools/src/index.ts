/**
 * `@sbo3l/cohere-tools` — SBO3L adapter for the Cohere agent / API surface.
 *
 ...
 *
 *   // Wire `tool.descriptor` into Cohere's tool registry; pair
 *   // each emitted tool call with `runSbo3lCohereToolCall`.
 *   ```
 */
...
/** APRP v1 input schema for Cohere's tool descriptor. Hand-authored — no schema-gen dep. */
...
export interface CohereDescriptor {
  name: string;
  description: string;
...
export interface SBO3LCohereTool {
  descriptor: CohereDescriptor;
  name: string;
  /** Throws `PolicyDenyError` on deny / requires_human, returns receipt on allow. */
  execute: (aprp: PaymentRequest) => Promise<PolicyReceipt>;
}

export function sbo3lCohereTool(options: SBO3LToolOptions): SBO3LCohereTool {
...
export async function runSbo3lCohereToolCall(
  tool: SBO3LCohereTool,
  call: CohereToolCall,
