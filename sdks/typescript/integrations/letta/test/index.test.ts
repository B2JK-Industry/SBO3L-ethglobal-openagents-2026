...
describe("sbo3lLettaTool", () => {
  it("default tool name is sbo3l_payment_request", () => {
    const t = sbo3lLettaTool({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
...
describe("runSbo3lLettaToolCall", () => {
  function call(name: string, input: object | string, id = "tc-001"): LettaToolCall {
  ...
  it("forwards allow into ok=true envelope", async () => {
    fetchOnce(allow("evt-1"));
    const t = sbo3lLettaTool({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
    const out = await runSbo3lLettaToolCall(t, call(t.name, aprpFixture));
  ...
  it("rejects malformed JSON input", async () => {
    const t = sbo3lLettaTool({ client: new SBO3LClient({ endpoint: ENDPOINT }) });
    const out = await runSbo3lLettaToolCall(t, call(t.name, "{not-json"));
    expect(out.ok).toBe(false);
    expect(JSON.parse(out.output).error).toBe("input.bad_arguments");
  });

  it("rejects mismatched tool name", async () => {
    const t = sbo3lLettaTool({ client: new SBO3LClient({ endpoint: ENDPOINT }), name: "pay" });
    const out = await runSbo3lLettaToolCall(t, call("not_pay", aprpFixture));
    expect(out.ok).toBe(false);
