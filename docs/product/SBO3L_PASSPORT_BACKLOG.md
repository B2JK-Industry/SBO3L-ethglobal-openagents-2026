# SBO3L Passport Two-Developer Backlog

**Goal:** turn the existing SBO3L implementation into a coherent
...
At the end of this backlog, SBO3L should demonstrate:

> An ENS-named AI agent calls SBO3L through MCP, receives a signed
...
| 2 | P2.1 | A | `sbo3l passport` CLI MVP | P1.1 | Capsule created from existing offline flow. |
| 2 | P2.2 | B | Trust-badge/operator-console capsule panels | P2.1 | Static UI renders capsule truthfully. |
| 3 | P3.1 | A | Functional `sbo3l-mcp` stdio server | P2.1 | MCP smoke test passes offline. |
...
- `schemas/sbo3l.passport_capsule.v1.json`
- `test-corpus/passport/*.json`
- `crates/sbo3l-cli/src/passport.rs`
- `crates/sbo3l-cli/src/main.rs`
- `crates/sbo3l-core` or a new small passport module if local pattern
  suggests it
- `scripts/validate_schemas.py`

**Acceptance criteria:**

- New schema id: `sbo3l.passport_capsule.v1`.
...
- `sbo3l passport verify --path <capsule>` exists and performs
...
- `crates/sbo3l-cli/src/passport.rs`
- `crates/sbo3l-cli/tests/passport_cli.rs`
- `demo-scripts/run-production-shaped-mock.sh`
...
- The command emits a valid `sbo3l.passport_capsule.v1`.
...
### P3.1 - Functional `sbo3l-mcp` Stdio Server

**Owner:** Developer A.

**Why:** KeeperHub explicitly values MCP/CLI/API, and MCP turns SBO3L
from a daemon into infrastructure other agents can call.

**Primary files:**

- `crates/sbo3l-mcp/Cargo.toml`
- `crates/sbo3l-mcp/src/main.rs`
- `crates/sbo3l-mcp/src/lib.rs`
- `crates/sbo3l-mcp/tests/*`
- `docs/cli/mcp.md`
...
- Tools use existing SBO3L libraries/CLI logic.
...
- A judge can understand how to call SBO3L from an agent tool.
...
**Why:** ENS must do real work. It should discover SBO3L proof
metadata, not just decorate a page.

**Primary files:**

- `crates/sbo3l-identity/src/*`
- `crates/sbo3l-cli/src/passport.rs`
- `crates/sbo3l-cli/tests/passport_cli.rs`
...
SBO3L should prove exactly what a live KeeperHub integration needs.

**Primary files:**

- `crates/sbo3l-keeperhub-adapter/src/lib.rs`
- `crates/sbo3l-keeperhub-adapter/tests/*`
...
```bash
SBO3L_KEEPERHUB_LIVE=1 bash demo-scripts/sponsors/keeperhub-live-smoke.sh
```
...
- One-pager tells the story:
  "KeeperHub executes; SBO3L proves the authorization."
...
**Why:** Uniswap reward cares about agentic finance. SBO3L should show
...
- One-pager frames SBO3L as a safety layer for agentic swaps.
...
```text
You are Developer B on SBO3L. Do not start final submission wiring
...
5. Start the next B-side task from docs/product/SBO3L_PASSPORT_BACKLOG.md.
...
- A developer can run an MCP tool and receive a SBO3L decision.
