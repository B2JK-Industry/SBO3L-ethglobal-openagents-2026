...
- **GuardedExecutor / ExecutionReceipt / ExecutionError moved to `mandate-core`** ([#49](https://github.com/B2JK-Industry/mandate-ethglobal-openagents-2026/pull/49), IP-4 prerequisite) — re-exported from `mandate-execution::lib` for back-compat. The standalone `mandate-keeperhub-adapter` crate (full IP-4) is still target; the type-move that unblocks it is on `main`.
