...
- **Mandate Passport capsule schema + verifier + CLI** (`schemas/mandate.passport_capsule.v1.json`, `crates/mandate-cli/src/passport.rs`, Passport P1.1 + P2.1) — `mandate passport run` drives the existing `POST /v1/payment-requests` pipeline in-process, builds a checkpoint, and self-verifies before atomic write; `mandate passport verify` runs the structural verifier against any capsule.
- **MCP-callable Mandate gateway** (`crates/mandate-mcp/`, Passport P3.1) — stdio JSON-RPC 2.0 server with `mandate.validate_aprp` / `mandate.decide` / `mandate.run_guarded_execution` / `mandate.verify_capsule` / `mandate.audit_lookup` (the IP-3 sister tool to KeeperHub's proposed `keeperhub.lookup_execution`). Sponsor demo: `bash demo-scripts/sponsors/mcp-passport.sh`. Judge-facing walk-through: `docs/mcp-integration-guide.md` (Passport P3.2).
- **KeeperHub `mandate_*` envelope helper** (`mandate_keeperhub_adapter::build_envelope`, Passport P5.1, IP-1) — composes `mandate_request_hash` / `mandate_policy_hash` / `mandate_receipt_signature` / `mandate_audit_event_id` directly from an existing `PolicyReceipt`. Helper shipped; live KeeperHub HTTP wiring still gated on KeeperHub publishing a stable submission/result schema (open questions in `docs/keeperhub-live-spike.md`).
- **Standalone `mandate-keeperhub-adapter` workspace crate** (IP-4) — `KeeperHubExecutor`, `GuardedExecutor` re-exports, `build_envelope`, README, changelog and runnable example live under `crates/mandate-keeperhub-adapter/`; `mandate-execution` re-exports it for back-compat. Crates.io publication remains a target, not a shipped claim.
