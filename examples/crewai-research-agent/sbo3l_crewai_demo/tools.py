"""Two callables the CrewAI agent reasons across:

  1. fetch_url — GET a JSON URL (research before paying)
  2. sbo3l_pay (built via sbo3l_crewai.sbo3l_tool) — APRP submit through
     SBO3L's policy boundary, routed to KeeperHub workflow
     m4t4cnpmhv8qquce3bv3c (live, verified 2026-04-30) when allowed.

Exposed as plain functions so consumers can wrap them with whichever
CrewAI primitive they prefer (`BaseTool` subclass, `@tool` decorator, etc.).
The README shows both wiring shapes.
"""
...
def fetch_url(url: str) -> str:
    ...
    Used as the agent's `data_fetch` tool — inspect a provider before
...
def build_sbo3l_pay_func(client: SBO3LClientSync) -> Any:
    """Return the descriptor's `func(input_str: str) -> str` — wire into a CrewAI BaseTool."""
