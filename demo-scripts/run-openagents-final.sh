#!/usr/bin/env bash
# SBO3L ETHGlobal Open Agents — final demo runner.
#
...
#      denied at the swap-policy guard AND at the SBO3L boundary.
...
./target/debug/sbo3l aprp validate test-corpus/aprp/golden_001_minimal.json >/dev/null
ok "golden_001_minimal.json passes schema"
if ./target/debug/sbo3l aprp validate test-corpus/aprp/adversarial_unknown_field.json 2>/dev/null; then
  fail "adversarial fixture must be rejected"
  exit 1
fi
ok "adversarial_unknown_field.json is rejected with schema.unknown_field"
./target/debug/sbo3l aprp run-corpus
echo

bold "3. Locked golden APRP request_hash"
EXPECTED=$(tr -d '\n' < test-corpus/aprp/golden_001_minimal.hash)
ACTUAL=$(./target/debug/sbo3l aprp hash test-corpus/aprp/golden_001_minimal.json)
...
./target/debug/sbo3l verify-audit --path test-corpus/audit/chain_v1.jsonl --skip-hash >/dev/null
ok "test-corpus/audit/chain_v1.jsonl: 3 events, seq + prev_event_hash + schema OK"
if ./target/debug/sbo3l verify-audit --path test-corpus/audit/chain_v1.jsonl 2>/dev/null; then
...
TEST_LOG="$(mktemp -t sbo3l-test-XXXXXX.log)"
...
ok "covers: APRP/hashing/signer/receipt/decision_token/audit (sbo3l-core)"
ok "covers: policy engine + expr evaluator + agent gate + paused-agents + budgets (sbo3l-policy)"
ok "covers: SQLite migrations + audit append + chain verify (sbo3l-storage)"
ok "covers: HTTP pipeline validate -> decide -> audit -> receipt (sbo3l-server)"
ok "covers: KeeperHub + Uniswap allow/deny + swap-policy guard (sbo3l-execution)"
ok "covers: ENS offline resolver + policy_hash verify (sbo3l-identity)"
echo
...
ok "ENS records resolve and policy_hash matches active sbo3l"
echo
...
ok "allow path -> uni-<ULID> execution_ref; deny path -> rejected at swap-policy guard AND SBO3L"
echo
...
TMPDIR="$(mktemp -d -t sbo3l-tamper-XXXXXX)"
...
with open(dst, "w") as fh:
    fh.writelines(lines)
PY
if ./target/debug/sbo3l verify-audit --path "$TMPDIR/tampered.jsonl" --skip-hash 2>/dev/null; then
...
if ./target/debug/sbo3l verify-audit --path "$TMPDIR/tampered.jsonl" 2>/dev/null; then
...
# inside sbo3l-server, behind the policy boundary.
#
...
if [[ "$NO_KEY_SOURCE_HITS" == "0" && "$NO_KEY_CARGO_HITS" == "0" && "$NO_KEY_FIXTURE_HITS" == "0" ]]; then
  ok "D-OA-12 Agent boundary: research-agent has no signer/private-key dependency; signing occurs inside SBO3L."
  NO_KEY_PROOF=PASS
else
...
cat <<EOF
  ...
  ✔ Prompt-injection denied at the SBO3L boundary; receipt + audit_event recorded.
