...
**Live ENS resolution is shipped + verified.** `LiveEnsResolver` (in `crates/sbo3l-identity/src/ens_live.rs`) reads the five `sbo3l:*` text records from a real Ethereum JSON-RPC endpoint. It is verified end-to-end against `sbo3lagent.eth` on **mainnet** (5 records on chain, owned by the team) and against the Sepolia OffchainResolver at `0x7c6913D52DfE8f4aFc9C4931863A498A4cACA8c3` (ENSIP-25 CCIP-Read flow, real signed responses from the Vercel-hosted gateway). The demo defaults to `OfflineEnsResolver` for CI determinism; visitors can flip to live with `SBO3L_ENS_RPC_URL=<rpc>` env.
```
...
**Mainnet broadcast UNI-A1 (LIVE):** A real Uniswap V3 swap is broadcast on Ethereum mainnet from the SBO3L deploy wallet (`0xdc7EFA…D231`) — 0.005 ETH → 11.5743 USDC, settled in block 25,013,950, gas 139,971 @ 2.19 gwei. Tx hash: `0xed68d1301b479c4229bc89cca5162b56517b80cbaeb654323e05b183000aff0b` ([Etherscan](https://etherscan.io/tx/0xed68d1301b479c4229bc89cca5162b56517b80cbaeb654323e05b183000aff0b)). The swap-policy guard + bounded recipient + slippage cap are the same primitives that protect any future agent-initiated swap; this broadcast proves the path end-to-end on production Ethereum.
```
